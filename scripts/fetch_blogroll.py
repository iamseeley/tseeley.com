...
def derive_homepage(url: str) -> str:
    url = url.rstrip("/")
    for suffix in [
        "/feed/atom", "/feed.atom", "/atom.xml", "/feed.xml", "/rss.xml",
        "/index.xml", "/feed/rss", "/feed/", "/feed", "/rss/", "/rss", "/atom/",
    ]:
...
def parse_opml(path: Path) -> list[dict]:
    ...
    def feeds(parent):
        ...
        for o in parent.findall("outline"):
            if xml_url := o.get("xmlUrl"):
                out.append({
                    "name": (o.get("text") or o.get("title") or "").strip() or "Untitled",
    ...
    blogroll = next(
        (o for o in root.iter("outline") if (o.get("text") or "").strip().lower() == "blogroll"),
        None,
    )
...
def write_toml(entries: list[dict], path: Path) -> None:
    ...
    for e in entries:
        ...
        if e["description"]:
            lines.append(f'description = "{toml_escape(e["description"])}"')
        lines.append("")
...
def main() -> None:
    ...
    print(f"Wrote {len(entries)} entries to data/blogroll.toml and static/blogroll.opml")
